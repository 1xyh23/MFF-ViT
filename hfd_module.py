...
#             num_heads=num_heads,
#             qkv_bias=qkv_bias,
#             attn_drop=attn_drop,  # 需要新增参数
#             proj_drop=proj_drop,
#             window_size=window_size,  # 对应论文中的s=7
#             alpha=alpha,  # 高低频头数比例
#         )

class HFDAttention(nn.Module):
    """
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., window_size=0,
        ...
        self.ws = window_size if window_size > 0 else 1

        # if self.use_rel_pos:
        #     H, W = input_size

        #     if self.h_heads > 0 and self.ws > 1:
        #             torch.zeros(2 * self.ws - 1, head_dim)
        #             torch.zeros(2 * self.ws - 1, head_dim))
        ...
        #     if self.l_heads > 0:
        #             torch.zeros(2 * H - 1, head_dim))
        #             torch.zeros(2 * W - 1, head_dim))
                
        #         if not rel_pos_zero_init:
        #             nn.init.trunc_normal_(self.global_rel_pos_h, std=0.02)
        #             nn.init.trunc_normal_(self.global_rel_pos_w, std=0.02)
...
#
#         if use_hfd:
#                 dim, num_heads, qkv_bias, use_rel_pos, rel_pos_zero_init, input_size
#             )
#         else:
#                 dim,
#                 num_heads=num_heads,
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        qkv_bias=True,
        drop_path=0.0,
        attn_drop=0.0,  # 需要新增参数
        proj_drop=0.0,  # 需要新增参数
        alpha=0.5,      # 新增，高低频头数比例
        norm_layer=nn.LayerNorm,
        act_layer=nn.GELU,
        use_rel_pos=False,
        rel_pos_zero_init=True,
        ...
        use_residual_block=False,
        use_hfd=True, 
        input_size=(64, 64),  # 需要传入当前阶段特征图尺寸
    ):
        """
        Args:
            dim (int): Number of input channels.
        ...
        self.use_hfd = use_hfd

        # 注意力模块选择
        self.attn = HFDAttention(
            dim=dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            attn_drop=attn_drop,  # 需要新增参数
            proj_drop=proj_drop,
            window_size=window_size,  # 对应论文中的s=8
            alpha=alpha,  # 高低频头数比例
        )
        # else:
    ...
    def forward(self, x):
        ...
        # === 第一阶段：hfd注意力 + 残差 ===
        # 层归一化1 -> hfd注意力
        x = self.norm1(x)  # 形状保持 [B, H, W, C]

        # 重塑为序列格式以适应标准注意力层
        B, H, W, C = x.shape
        x_seq = x.reshape(B, H * W, C)  # [B, N, C], N=H*W

        # hfd注意力处理（需要传递H/W参数）
        attn_out = self.attn(x_seq, H, W)  # 输出形状 [B, N, C]

        # 恢复特征图格式
        attn_out = attn_out.reshape(B, H, W, C)  # [B, H, W, C]

        # 带DropPath的残差连接
        x = shortcut + self.drop_path(attn_out)  # 残差连接1

        # === 第二阶段：MLP + 残差 ===
        # 层归一化2 -> MLP
        shortcut_mlp = x  # 保存MLP前的状态
        x = self.norm2(x)  # 形状保持 [B, H, W, C]

        # MLP处理（需转换为序列格式）
        # x_mlp = x.reshape(B, H * W, C)  # [B, N, C]
        mlp_out = self.mlp(x)  # MLP输出 [B, N, C]
        # mlp_out = mlp_out.reshape(B, H, W, C)  # 恢复形状

        # 带DropPath的残差连接
        x = shortcut_mlp + self.drop_path(mlp_out)  # 残差连接2

        # === 可选残差块 ===
        if self.use_residual_block:
            # 调整维度顺序以匹配卷积层 [B, C, H, W]
            x = x.permute(0, 3, 1, 2)  # [B, C, H, W]
            x = self.residual(x)  # 残差块处理
    ...
    #         x = self.attn(x, H, W)  # 必须传入H,W参数
    #         x = x.reshape(B, H, W, C)  # 恢复形状
    #         x = shortcut + x  # 残差连接
    #     else :
    #         shortcut = x
    #         x = self.norm1(x)
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        attn_drop_rate=0.0,  # 新增
        proj_drop_rate=0.0,  # 新增
        hfd_alpha=0.5,  # 新增
        hfd_block_indexes=(),  # 新增参数
        hfd_window_size=0,  # 新增独立参数
        ...
        self.hfd_alpha = hfd_alpha
        print("################################################")
        print("hfd_alpha是：", hfd_alpha)
        print("depth是：", depth)
        print("hfd_window_size是：", hfd_window_size)
        print("hfd_block_indexes是：",hfd_block_indexes)
        print("################################################")
        ...
        for i in range(depth):
            # current_size = (img_size // patch_size) // (2 ** (i // 4)) if dynamic_size else (img_size // patch_size)
            current_size = img_size // patch_size
            block = Block(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
                ...
                attn_drop=0.0,  # 需要新增参数
                proj_drop=0.0,  # 需要新增参数
                alpha=hfd_alpha,  # 新增，高低频头数比例
                norm_layer=norm_layer,
                act_layer=act_layer,
                use_rel_pos=use_rel_pos,
                rel_pos_zero_init=rel_pos_zero_init,
                # window_size=window_size if i in window_block_indexes else 0,
                window_size=hfd_window_size if i in hfd_block_indexes else 0,  # 使用独立参数
                use_residual_block=i in residual_block_indexes,
                use_hfd=True,  #新增是否使用hfd
                # input_size=(img_size // patch_size, img_size // patch_size),
                input_size=(current_size, current_size),  # 动态传递
            )
    ...
    def mae_forward_2(self, x):
        x = x.permute(0, 2, 3, 1)
        for blk in self.blocks:
            x = blk(x)
        return {'last_feat':x.permute(0, 3, 1, 2)}
